...
def introspect_db(
    ...
    if not db_url:
        raise ValueError(
            "Database URL not provided and get_db_url() did not return one."
        )
    ...
    try:
        ...
        raise ConnectionError(
            f"Failed to connect to database or reflect metadata: {e}"
        ) from e
    ...
    schema_info["data_model_summary"] = generate_data_model_summary(
...
def detect_implicit_relationships(
    ...
    for table_name in all_tables:
        table_cols = [
            col["name"] for col in schema_info["tables"][table_name]["columns"]
        ]
        ...
        for col_name in table_cols:
            ...
            for other_table in all_tables:
                if not primary_keys_registry.get(
                    other_table
                ):  # Skip if target table has no PK
                    continue

                # Basic singular/plural matching (case-insensitive for robustness)
                # (e.g., user_id -> users table, or users_id -> user table)
                singular_other_table = (
                    other_table[:-1]
                    if other_table.lower().endswith("s")
                    else other_table
                )
                ...
                if match:
                    ...
                    is_explicit = any(
                        ...
                        and rel["from_column"] == col_name
                        and rel["to_table"] == other_table
                        for rel in schema_info["relationships"]["explicit"]
                    )
                    if is_explicit:
                        break  # Already an explicit FK, don't add as implicit

                    if primary_keys_registry[
                        other_table
                    ]:  # Ensure PK list is not empty
                        schema_info["relationships"]["implicit"].append(
                            {
                                "from_table": table_name,
                                "from_column": col_name,
                                "to_table": other_table,
                                "to_column": primary_keys_registry[other_table][
                                    0
                                ],  # Assume first PK column
...
def collect_stats_and_samples(
    ...
    for table_name in all_tables:
        ...
        if table_name not in metadata.tables:
            print(
                f"Warning: Table '{table_name}' not found in metadata for stats/sampling. Skipping."
            )
            continue
        ...
        try:
            ...
            row_count_query = select(func.count().label("total_rows")).select_from(
                table_obj
            )
            ...
            current_table_info["row_count"] = (
                row_count_val if row_count_val is not None else 0
            )
        except Exception as e:
        ...
        if (
            include_sample_data
            and isinstance(current_table_info["row_count"], int)
            and current_table_info["row_count"] > 0
        ):
            try:
...
def generate_table_descriptions(
    schema_info: Dict[str, Any], include_indexes: bool
) -> None:
    """
    ...
    for table_name, table_data in schema_info["tables"].items():
        ...
        for col in table_data["columns"]:
            ...
            columns_desc_parts.append(
                f"{col['name']}: {col['type']}{nullable_info}{pk_marker}{default_info}{comment_info}"
            )
        ...
        if include_indexes and table_data["indexes"]:  # Check include_indexes again
            ...
            for idx in table_data["indexes"]:
                ...
                idx_descs_parts.append(
                    f"{idx['name']} on ({', '.join(idx['columns'])}){unique_marker}"
                )
...
def generate_data_model_summary(
    all_tables: List[str], schema_info: Dict[str, Any]
) -> str:
    """
    ...
    for table_name_key in all_tables:  # Use sorted list for consistent order
        ...
        pk_cols_str = (
            ", ".join(table_detail["primary_keys"])
            if table_detail["primary_keys"]
            else "None"
        )
    ...
    return "\n".join(
        [
            ...
            "\nTABLE DETAILS:",
            (
                "\n".join(tables_summary_list)
                if tables_summary_list
...
def format_schema_outputs(
    schema_info: Dict[str, Any], pretty_json: bool = True
) -> Dict[str, str]:
    """
