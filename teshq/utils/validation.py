    ...
    def validate_database_url(db_url: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if parsed.scheme not in ConfigValidator.SUPPORTED_DB_TYPES:
                return False, f"Unsupported database type: {parsed.scheme}. Supported: {', '.join(ConfigValidator.SUPPORTED_DB_TYPES)}"
    ...
    def validate_config(config: Dict[str, Any]) -> List[str]:
        ...
        for path_key in ["OUTPUT_PATH", "FILE_STORE_PATH"]:
            if path_key in config:
                is_valid, message = ConfigValidator.validate_file_path(
                    config[path_key], must_be_writable=True
                )
                if not is_valid:
    ...
    def validate_save_path(save_path: str, format_type: str) -> Tuple[bool, str]:
        ...
        if format_type in expected_extensions:
            if extension not in expected_extensions[format_type]:
                return False, f"File extension {extension} doesn't match format {format_type}. Expected: {', '.join(expected_extensions[format_type])}"
