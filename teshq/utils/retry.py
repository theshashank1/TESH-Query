...
from typing import Any, Callable, List, Optional, Type, Union  # noqa: F401
...
def retry_with_backoff(config: Optional[RetryConfig] = None, operation_name: str = "operation"):
    """
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(1, config.max_attempts + 1):
                try:
                    ...
                    if attempt > 1:
                        logger.success(
                            f"Succeeded {operation_name} after retries", attempt=attempt, operation_name=operation_name
                        )
...
class AsyncRetryManager:
    ...
    async def execute_with_retry(self, operation: Callable, operation_name: str = "async_operation", *args, **kwargs) -> Any:
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            try:
                ...
                if attempt > 1:
                    logger.success(
                        f"Succeeded async {operation_name} after retries", attempt=attempt, operation_name=operation_name
                    )
