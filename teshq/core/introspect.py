...
def introspect_db(
    ...
    save_schema_to_files(schema_info, "schema.json", "schema.txt")
...
def detect_implicit_relationships(
    ...
    for table_name in all_tables:
        ...
        for col_name in table_cols:
            ...
            elif col_name.lower().endswith("id") and len(col_name) > 2 and col_name[-2].islower():
                potential_table_root = col_name[:-2]
            else:
                continue

            for other_table in all_tables:
                if not primary_keys_registry.get(other_table):
                    continue
                ...
                if match:
                    ...
                    if primary_keys_registry[other_table]:
                        schema_info["relationships"]["implicit"].append(
                            {
                                "from_table": table_name,
                                "from_column": col_name,
                                "to_table": other_table,
                                "to_column": primary_keys_registry[other_table][0],
                                "relationship_type": "potential-many-to-one",
                                "confidence": "medium",
                            }
                        )
                        break
...
def collect_stats_and_samples(
    ...
    for table_name in all_tables:
        if table_name not in metadata.tables:
            ...
            continue
        table_obj = cast(Table, metadata.tables[table_name])
        ...
        try:
            ...
            current_table_info["row_count"] = "Unknown"
            ...
            try:
                ...
                for row in result:
                    ...
                    for col_key, value in row._asdict().items():
                        col_name_str = str(col_key)
                        if value is not None:
                            try:
                                json.dumps(value)
                                sample_row[col_name_str] = value
                            except (TypeError, OverflowError):
                                sample_row[col_name_str] = str(value)
                        else:
                ...
                current_table_info["sample_data"] = []
...
def generate_table_descriptions(schema_info: Dict[str, Any], include_indexes: bool) -> None:
    ...
    for table_name, table_data in schema_info["tables"].items():
        ...
        if include_indexes and table_data["indexes"]:
            idx_descs_parts = []
...
def generate_data_model_summary(all_tables: List[str], schema_info: Dict[str, Any]) -> str:
    ...
    """
    tables_summary_list = []
    for table_name_key in all_tables:
    ...
    if not relationships_summary_list:
...
def save_schema_to_files(
    ...
    output_dir = storage_paths.schema
    ...
    return str(json_path) if json_path else None, str(text_path) if text_path else None  # type: ignore
...
def main(
    ...
    try:
        ...
        json_path, text_path = save_schema_to_files(schema_info)
